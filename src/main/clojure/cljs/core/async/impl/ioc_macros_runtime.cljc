(ns cljs.core.async.impl.ioc-macros-runtime
  ...
  #?(:cljs (:require-macros [cljs.core.async.impl.ioc-macros :as ioc]))
  #?(:clj (:import [java.util.concurrent.locks Lock])))
  ...
  (terminate-block [this state-sym _]
    `(do (case ~val-id
           ~@(concat (mapcat (fn [test blk]
                               `[~test (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym
                                         ~STATE-IDX ~blk)])
                       test-vals jmp-blocks)
               (when default-block
                 `[(do (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~STATE-IDX ~default-block)
                       :recur)])))
         :recur)))
...
(defrecord Jmp [value block]
  ...
  ITerminator
  (terminate-block [this state-sym _]
    `(do (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~VALUE-IDX ~value ~STATE-IDX ~block)
         :recur)))
...
(defrecord Return [value]
  ...
  (terminate-block [this state-sym custom-terminators]
    (if-let [f (get custom-terminators (terminator-code this))]
      `(~f ~state-sym ~value)
      `(do (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym
             ~VALUE-IDX ~value
             ~STATE-IDX :finished)
           nil))))
...
(defrecord CondBr [test then-block else-block]
  ...
  (terminate-block [this state-sym _]
    `(do (if ~test
           (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym
...
(defn fixup-aliases [sym env]
  (let [aliases #?(:clj (ns-aliases *ns*) :cljs [])]        ;; todo implement by looking into compiler state
...
(defn- build-new-state [local-map idx state-sym blk]
  (let [results (->> blk
    ...
    (if-not (empty? results)
      `(cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~@results)
      state-sym)))
                     ...
                     (let [~state-val-sym (aget ~state-sym ~STATE-IDX)]
                       (cond
                         ~@(mapcat
                             ...
                             (:blocks machine)))))]
       (fn state-machine#
         ([] (cljs.core.async.impl.ioc-macros/aset-all! (make-array ~state-arr-size)
          ...
          (let [ret-value# (try (loop []
                                  (let [result# (switch# ~state-sym)]
                                    (if (cljs.core/keyword-identical? result# :recur)
                                      ...
                                      result#)))
                                (catch js/Object ex#
                                  (cljs.core.async.impl.ioc-macros/aset-all! ~state-sym ~CURRENT-EXCEPTION ex#)
                                  (cljs.core.async.impl.ioc-helpers/process-exception ~state-sym)
                                  :recur))]
